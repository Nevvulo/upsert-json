            ...
            if (Array.isArray(sourceJson)) {
                if (filterProperty) {
                    ...
                    if (!isObjectArray(sourceJson))
                        return core.setFailed(`"filterProperty" was specified, but not all elements satisfy the type 'object'`);
                    // Do all of the objects in the source array share [filterProperty]
                    // as a common property?
                    const propertiesEqual = sourceJson.every(object => filterProperty in object);
                    if (!propertiesEqual)
                        return core.setFailed(`Not all objects in the source array contain the key "${filterProperty}"`);
                    ...
                    if (!inputObjectHasProp)
                        return core.setFailed(`Input object does not contain the key "${filterProperty}"`);
                    const output = (0, lodash_1.values)((0, lodash_1.merge)((0, lodash_1.keyBy)(sourceJson, filterProperty), (0, lodash_1.keyBy)([inputJson], filterProperty)));
                    return core.setOutput('json', output);
                }
                else {
                    const outputArray = (0, lodash_1.cloneDeep)(sourceJson);
                ...
                if (typeof inputJson !== 'object')
                    return core.setFailed('input_json must be an object if source_json is an object');
                return core.setOutput('json', Object.assign(Object.assign({}, sourceJson), inputJson));
            }
